...
from allegro.utils.allegro_api_utils import (
    get_categories,
    get_categories_by_parent_id,
    search_listings_in_allegro,
...
def search_listings() -> Response:
    try:
        ...
        listings = search_listings_in_allegro(
            device_name, token.get("access_token", "")
        )
...
def get_category_by_parent_id() -> Response:
    try:
        ...
        token: Dict[str, Any] = get_access_token()
        categories = get_categories_by_parent_id(
            token.get("access_token", ""), parent_id
        )
        return jsonify(categories)
    except Exception as e:
        return Response(str(e), status=500)
